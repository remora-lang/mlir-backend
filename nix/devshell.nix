# Development shell: everything needed to build, run, test and debug the
# project locally.
{ pkgs, iree, mlir-backend }:
...
let
  ...
  build = pkgs.writeShellScriptBin "build" ''
    set -euo pipefail
    jobs=''${1:-}
    ${pkgs.lib.optionalString (cudaDepsDir != "") ''export IREE_CUDA_DEPS_DIR=${cudaDepsDir}''}
